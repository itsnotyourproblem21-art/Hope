...
const SubjectPagesApp = ({ activeSubject }) => {
	...
	const handleSingleTestReset = (testId) => {
		setTestData(prevData =>
			prevData.map(test => {
				if (test.id === testId) {
					return { ...test, attempts: 0, history: [] };
				}
				return test;
			})
		);
	...
	const displayDate = isUnattempted ? 'N/A' : selectedMetrics.date;
							...
							<div className="flex justify-end space-x-3 pt-4 mt-auto" style={{ borderTopColor: borderColorDark, borderTopWidth: '1px', borderTopStyle: 'solid' }}>
								...
								<button
									...
									onClick={() => {
										...
										// Only allow starting Test 1 (index 0) for now, as that's the only test with questions
										if (testIndex !== 0) {
											showMessage(`Test ${selectedTest?.id} is not yet available. Only Test 1 is currently functional.`);
											console.warn(`Test ${selectedTest?.id} not available - only Test 1 (index 0) has questions`);
											return;
										}
