...
function updateDashboardOffset() {
  ...
  document.documentElement.style.setProperty('--dashboard-offset', `${marginLeft}px`);
}
...
function reviewTestFromSubject(subjectName, testIndex) {
    ...
    if (!loadCompletedTestState(subjectName, testIndex)) {
    ...
    lastResultRecord = attempts[attempts.length - 1];
...
function showView(viewId, event, navElement) {
    ...
    const reviewViews = ['review-view', 'detailed-review-view', 'results-view'];
    ...
    if (examViews.includes(viewId)) {
        ...
        if (sidebar) {
            ...
            // Restore sidebar width if it was collapsed
...
function showResults() {
    ...
    refreshResultsSummaryUI(lastAttemptList, lastResultRecord);
    updateResultsAttemptCard(attemptRecord, attempts);
    
    // Display individual question results
    updateResultsAttemptModal(attemptRecord, attempts);
}
...
function filterTaggedQuestions(filter) {
    ...
    // Filter items
    taggedReviewFilteredItems = taggedReviewAllItems.filter(item => {
...
function updateTaggedQuestionList() {
    ...
    const totalCountEl = document.getElementById('tagged-total-count');
    if (totalCountEl) totalCountEl.textContent = taggedReviewFilteredItems.length;
    ...
    taggedReviewFilteredItems.forEach((item, index) => {
        ...
        listItem.innerHTML = `
            ...
            <span class="flex-1 text-sm">${item.subject} #${item.testIndex + 1} Q${item.questionIndex + 1}</span>
...
function updateTaggedReview() {
    ...
    const isCorrect = currentItem.userAnswer === question.a;
    const userAnswerLetter = currentItem.userAnswer !== undefined ? String.fromCharCode(65 + currentItem.userAnswer) : '--';
    ...
    document.getElementById('tagged-subject-info').textContent = `${currentItem.subject} Test #${currentItem.testIndex + 1}`;
...
function showPreviousTests(event) {
    ...
    const countEl = document.getElementById('previous-tests-count');
    const metaEl = document.getElementById('previous-tests-card-meta');

    if (!tableBody || !emptyState || !countEl) {
        console.warn('Previous tests view elements missing');
        return;
    }

    // Collect all attempts from storage
    const history = loadTestHistory();
    const allAttempts = [];
    ...
    if (allAttempts.length === 0) {
        ...
        countEl.textContent = 'No completed tests yet';
        if (metaEl) metaEl.textContent = 'Complete a practice exam to see it listed here';
        return;
    }

    emptyState.style.display = 'none';
    const attemptWord = allAttempts.length === 1 ? 'attempt' : 'attempts';
    countEl.textContent = `${allAttempts.length} test ${attemptWord} recorded`;
    ...
    allAttempts.forEach((attempt, index) => {
        const rowNumber = allAttempts.length - index;
        const subject = attempt.subject || 'Unknown Subject';
        const testIndex = (attempt.testIndex ?? 0) + 1;
        const testName = `${subject} Test #${testIndex}`;
        const rawPercent = typeof attempt.score === 'number' && !Number.isNaN(attempt.score) ? Math.round(attempt.score) : null;
        const rawDisplay = rawPercent !== null ? `${rawPercent}%` : '--';
        const estimatedScore = rawPercent !== null ? getScaledScore(rawPercent) : '--';
        const estimatedDisplay = estimatedScore === '--' ? '--' : estimatedScore;
        const dateDisplay = attempt.date ? formatResultDate(attempt.date) : '--';

        const row = document.createElement('tr');
        row.innerHTML = `
            <td class="previous-tests-index">${rowNumber}</td>
            <td class="previous-tests-raw-score">${rawDisplay}</td>
            <td>${escapeHtml(testName)}</td>
            <td>${buildSubjectPill(subject)}</td>
...
const subjects = [
  ...
  { name: 'Reading Comprehension', icon: 'BookOpen', description: '', duration: '50 minutes per test', color: 'text-green-800', bgColor: 'bg-green-100' },
...
const App = () => {
  ...
  return React.createElement('div', {
    ...
    // Header with bubbly pattern that extends to the sidebar edge
    React.createElement('div', {
    ...
    // Centered Subject Grid Container
    React.createElement('div', {
      className: 'flex-1 w-full flex items-center justify-center px-4 sm:px-6 lg:px-8 pb-16'
    }, [
      React.createElement('div', {
        className: 'bg-white rounded-3xl border-4 border-gray-900 p-6 md:p-10 shadow-2xl max-w-7xl w-full',
        style: {
        ...
        React.createElement('main', {
          ...
          React.createElement(SubjectCard, { key: index, subject: subject })
        ))
      ])
    ])
  ]);
